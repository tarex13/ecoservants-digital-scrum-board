...
class EcoServants_Scrum_Board_API extends WP_REST_Controller {
    ...
    // ──────────────────────────────────────────────
    // GET /tasks — List tasks with pagination, filters, and labels
    // ──────────────────────────────────────────────
    public function get_items( $request ) {
        ...
        foreach ( $filters as $param => $format ) {
            ...
            if ( ! empty( $value ) ) {
                $where .= " AND t.{$param} = {$format}";
                $args[] = $value;
            }
        }

        // Main query with labels
        $sql = "
            SELECT
                t.*,
                GROUP_CONCAT(
        ...
        /*
        ---------------------------------------
        Convert labels string into structured array
        ---------------------------------------
        */
        ...
        /*
        ---------------------------------------
        Count query (without joins for speed)
        ---------------------------------------
        */
        ...
        foreach ( $filters as $param => $format ) {
            ...
            if ( ! empty( $value ) ) {
                $count_where .= " AND {$param} = {$format}";
                $count_args[] = $value;
            }
        }

        $count_sql = "SELECT COUNT(*) FROM {$tasks_table} {$count_where}";

        if ( ! empty( $count_args ) ) {
            $total = (int) $db->get_var( $db->prepare( $count_sql, $count_args ) );
        } else {
            $total = (int) $db->get_var( "SELECT COUNT(*) FROM {$tasks_table}" );
        }

        return EcoServants_API_Response::paginated(
            $tasks,
            $total,
            $pag['page'],
            $pag['per_page']
